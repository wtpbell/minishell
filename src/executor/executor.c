...
{	
	...
	else if (node->redirections->type == TOKEN_APPEND || node->redirections->type == TOKEN_HEREDOC
		|| node->redirections->type == TOKEN_REDIR_IN || node->redirections->type == TOKEN_REDIR_OUT)
		return (exec_redir(node, env));
